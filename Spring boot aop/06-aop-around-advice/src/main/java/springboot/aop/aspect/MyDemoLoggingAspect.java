...
public class MyDemoLoggingAspect {
    ...
    @Around("execution(* springboot.aop.service.*.getFortune(..))")
    public Object aroundGetFortune(ProceedingJoinPoint theProceedingJoinPoint) throws Throwable {

        // pritn out method we are advesing on
        ...
        Object result = null;
        try {
            result = theProceedingJoinPoint.proceed();     // ProceedingJoinPoint(only if @Around) handle to target method and proceed() execute the target method
          ...
          /*  // give user a custom message
            result = "Major Accident but no worries your private jet is on the way";
        */
    ...
    @After("execution(* springboot.aop.dao.*.*(..))")
    // @After will be executed regardless of your code fails or succeeds
    ...
    @AfterReturning(   // we can change the value of returned value or object with this annotation
            pointcut = "execution(* springboot.aop.*.*.*(..))",
            returning = "result")
